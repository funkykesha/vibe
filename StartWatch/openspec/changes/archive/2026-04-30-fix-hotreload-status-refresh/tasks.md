...
- [x] 3.3 Manual test: reset config to 4 services, start daemon, add 5th service via jq, wait 2s, verify `startwatch status` shows 5 services
- [x] 3.4 Manual test: save invalid config (remove `check.value`), wait 2s, verify status still shows previous valid config
