...
- [x] 7.2 Edit config file with VSCode (atomic save) → daemon reloads exactly once
- [x] 7.3 Kill a monitored service → notification appears with failure reason
- [x] 7.4 Restart the service → "Service Recovered" notification appears
- [x] 7.5 Save invalid config (empty service name) → "Config Error" notification appears
- [x] 7.6 Rapid double-save (within 200ms) → single reload, not two
- [x] 7.7 Restart daemon with service already down → no notification on startup
