...
- `/usr/local/bin/startwatch` wrapper must use double quotes for path assign: `MENU_BIN="/path/..."`
- LaunchAgent must run bundle binary: `.../StartWatchMenu.app/Contents/MacOS/startwatch daemon`, else possible `AppleSystemPolicy: load code signature error 2`
