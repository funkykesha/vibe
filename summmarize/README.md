...
Assumes `lib/eliza-client` exists one level up. Adjust `package.json` path if your layout differs.
...
- **Model**: edit `ENRICH_MODEL` in `.env`. Use a cheap one for bulk; swap to Claude/GPT for quality reruns.
