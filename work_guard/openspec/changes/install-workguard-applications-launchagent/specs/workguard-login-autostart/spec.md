...
The WorkGuard LaunchAgent SHALL open `/Applications/WorkGuard.app` rather than executing Python or a project-local app directly.

#### Scenario: LaunchAgent ProgramArguments are fixed
- **WHEN** the WorkGuard LaunchAgent plist is generated
- **THEN** `ProgramArguments` contains exactly `open` followed by `/Applications/WorkGuard.app`
