...
### Requirement: Settings dialog includes overlay lock fields

**Reason**: Lock seconds, overlay delay, notification interval, and the work-apps whitelist are removed from the user-editable settings surface. They are either fixed module-level constants (`OVERLAY_FIRST_DELAY_MIN`, `LOCK_INITIAL_SEC`, `LOCK_MAX_SEC`, `NOTIFY_INTERVAL_MIN` in `work_guard.py`) or removed features (work-apps). The dialog is reduced to schedule fields only.

**Migration**: On first launch after upgrade, the legacy config loader silently drops `overlay_lock_initial_sec`, `overlay_lock_max_sec`, `overlay_delay_min`, `notification_interval_min`, and `work_apps` from `config.json`. No user action is required.
