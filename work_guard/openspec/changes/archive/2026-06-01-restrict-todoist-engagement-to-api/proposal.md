...
- `todoist_signals.py` — `BrowserHistoryReader` becomes dead code; remove it together with its reader-only constants and helper (`_BROWSER_HISTORY_PATHS`, `_HISTORY_THROTTLE_SEC`, `_chrome_time_to_dt`). `TodoistApiClient` unchanged.
...
- No change to the overlay, the dashboard shape, the threshold/cadence logic, or the API request surface. Privacy boundary tightens (one fewer local reader). The now-archived `redesign-todoist-overlay` change also touched `engagement_monitor.py` (snapshot shape) and `todoist_signals.py` (`dashboard()`), but the two edits are disjoint (this one removes signal sources; that one reshaped dashboard data) and it has already landed.
