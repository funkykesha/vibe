...
- **P0 contract:** Generate and reload one LaunchAgent: `com.agaibadulin.workguard`, running `/usr/bin/open /Applications/WorkGuard.app` with `RunAtLoad=true` and `KeepAlive=false`.
...
5. Refresh LaunchServices explicitly during reinstall with `lsregister`, user domains only.
   ...
   - `lsregister -f /Applications/WorkGuard.app` — force-register the new bundle (updates LaunchServices metadata for the replaced path).
   - **Note:** Apple removed `lsregister -kill` on recent macOS (unsafe / no longer useful). Do not depend on a full database flush; unregister + force-register is the supported path.
...
6. Generate LaunchAgent from the rebuild flow and reload it with user-domain `launchctl`.
   ...
   Rationale: the LaunchAgent payload is part of the install contract, not runtime state. Rebuild should write the plist with `ProgramArguments` exactly `/usr/bin/open` and `/Applications/WorkGuard.app`, then run `launchctl bootout gui/<uid> <plist> || true` and `launchctl bootstrap gui/<uid> <plist>` so loaded state matches disk.
...
- LaunchServices can keep stale metadata after replacement -> run explicit `lsregister -u` (before replace when the old bundle exists) and `lsregister -f` on the new bundle; avoid removed `-kill` flags on modern macOS.
...
- Plist `ProgramArguments` are exactly `/usr/bin/open` and `/Applications/WorkGuard.app`.
...
- None. The packaging directory is `packaging/`.
