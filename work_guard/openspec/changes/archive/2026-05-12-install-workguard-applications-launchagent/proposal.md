...
- Fix the LaunchAgent contract to `~/Library/LaunchAgents/com.agaibadulin.workguard.plist`, launching via `/usr/bin/open /Applications/WorkGuard.app`, with `RunAtLoad=true` and `KeepAlive=false`.
...
- `workguard-login-autostart`: WorkGuard can start at login from `~/Library/LaunchAgents/com.agaibadulin.workguard.plist`, which runs `/usr/bin/open /Applications/WorkGuard.app` with `RunAtLoad=true` and `KeepAlive=false`.
